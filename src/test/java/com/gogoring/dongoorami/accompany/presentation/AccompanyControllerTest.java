...
class AccompanyControllerTest {
    ...
    void success_getAccompanyCommentsByMemberFirst() throws Exception {
        // given
        Member member = MemberDataFactory.createLoginMember();
        memberRepository.save(member);
        String accessToken = tokenProvider.createAccessToken(member.getProviderId(),
        ...
        // when
        ResultActions resultActions = mockMvc.perform(
                get("/api/v1/accompanies/comments/my-page").header(
                                "Authorization", accessToken)
                        .param("size", String.valueOf(size))
        );
        ...
        // then
        resultActions.andExpect(status().isOk())
                .andDo(document("{ClassName}/getAccompanyCommentsByMemberFirst",
                        preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()),
                        queryParameters(
                                parameterWithName("size").description(
                                        "조회할 동행 구인 댓글 개수, 값 넣지 않으면 기본 10개").optional()
                        ),
                        responseFields(
                                fieldWithPath("hasNext").type(BOOLEAN)
                                        .description("다음 동행 구인 댓글 정보 존재 여부"),
                                fieldWithPath("accompanyCommentShortResponses").type(ARRAY)
                                        .description("동행 구인 댓글 목록"),
                                fieldWithPath(
                                        "accompanyCommentShortResponses[].accompanyCommentId").type(
                                        NUMBER).description("동행 구인 댓글 아이디"),
                                fieldWithPath(
                                        "accompanyCommentShortResponses[].accompanyPostId").type(
                                        NUMBER).description("댓글 단 동행 구인글 아이디"),
                                fieldWithPath(
                                        "accompanyCommentShortResponses[].accompanyPostTitle").type(
                                        STRING).description("동행 구인글 제목"),
    ...
    @Test
    @WithCustomMockUser
    @DisplayName("특정 회원이 작성한 동행 구인 댓글 목록을 조회할 수 있다. - 이후 요청")
    void success_getAccompanyCommentsByMemberAfterFirst() throws Exception {
        // given
        Member member = MemberDataFactory.createLoginMember();
        memberRepository.save(member);
        String accessToken = tokenProvider.createAccessToken(member.getProviderId(),
        ...
        // then
        resultActions.andExpect(status().isOk())
                .andDo(document("{ClassName}/getAccompanyCommentsByMemberAfterFirst",
                        preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()),
                        queryParameters(
                                parameterWithName("cursorId").description("마지막으로 받은 동행 구인 댓글 아이디"),
                                parameterWithName("size").description(
                                        "조회할 동행 구인 댓글 개수, 값 넣지 않으면 기본 10개").optional()
                        ),
                        responseFields(
